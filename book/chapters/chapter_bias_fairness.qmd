...
The same observed score difference can reflect impact, bias, or some combination of both. A test score gap between two groups is not, by itself, evidence of bias. Nor is the absence of a gap evidence of fairness. Determining which is present requires evidence beyond the score distribution, and that evidence is the subject of the rest of this chapter. See @fig-bias-impact.

```{r fig-bias-impact, echo=FALSE, fig.height=3.8, fig.width=8.5, fig.cap="An observed group mean difference in test scores. The question is what produces it: genuine differences in the construct (impact), construct-irrelevant test features (bias), or both."}
...
Examples of uniform and non-uniform DIF are available in @fig-uniform-nonuniform.

```{r fig-uniform-nonuniform, echo=FALSE, fig.height=4.2, fig.width=8.5, fig.cap="Uniform DIF (left): one group consistently outperforms the other at all ability levels. Non-uniform DIF (right): the group difference changes direction across ability levels. MH detects uniform DIF only; logistic regression detects both."}
...
Two classical (non-IRT) methods dominate applied DIF analysis: the Mantel–Haenszel procedure and logistic regression. Both condition on total test score as a proxy for ability. Both are available in R through the `difR` package [@magisGeneralFrameworkPackage2010].

### The Mantel–Haenszel Procedure

The Mantel–Haenszel (MH) procedure [@hollandthayer1986] was developed at Educational Testing Service and became the dominant method for large-scale DIF detection . Its prevalence in applied testing reflects historical adoption and practical interpretability more than statistical superiority, a point we return to below.
...
Within a single stratum, the comparison looks like @tbl-mh-stratum:

```{r tbl-mh-stratum, echo=FALSE}
mh_ex <- tribble(
...
Three statistics matter in @tbl-mh-results:
...
Logistic regression DIF [@swaminathanDetectingDifferentialItem1990] extends MH by modeling the probability of correct response as a function of total score, group membership, and their interaction. It is statistically more capable than MH and is the recommended method when software is available.

Three nested models are estimated for each item and provided in @tbl-lr-models:
...
The `type = "both"` argument tests for both uniform and non-uniform DIF simultaneously. Setting `type = "udif"` tests uniform DIF only (equivalent in scope to MH); `type = "nudif"` tests non-uniform DIF only. In most applied situations `"both"` is the right choice.
...
**Item 7.** *"Which word best completes the sentence: 'The scientist's findings were met with widespread \_\_\_\_\_ from the academic community.'"* Suppose this item flags as Class C uniform DIF favoring the reference group (majority English speakers). Does that mean it is biased?
...
```{r fig-pred-bias, echo=FALSE, fig.height=3.5, fig.width=8, fig.cap="Differential prediction with equal slopes but different intercepts. The test predicts GPA with similar accuracy for both groups, but systematically underpredicts actual GPA for Group B at every score level."}
#| fig-alt: "Scatterplot of first-year GPA (y-axis) against test score (x-axis) for two groups. Group A (dark blue) and Group B (orange) show similarly-sloped regression lines with confidence bands, but the Group B line sits consistently higher than Group A. A double-headed arrow at score = 60 marks the vertical gap between the two lines, labeled 'Intercept difference', illustrating that Group B students are systematically underpredicted at every score level."
set.seed(99)
n_a <- 250; n_b <- 250

score_a <- rnorm(n_a, 62, 8)
score_b <- rnorm(n_b, 55, 8)
gpa_a   <- pmax(0, pmin(4, 1.20 + 0.028 * score_a + rnorm(n_a, 0, 0.42)))
gpa_b   <- pmax(0, pmin(4, 1.58 + 0.028 * score_b + rnorm(n_b, 0, 0.42)))
...
mean_a <- round(mean(score_a), 1)
mean_b <- round(mean(score_b), 1)
...
ggplot(df_pred, aes(x = score, y = gpa, color = group)) +
  ...
  annotate("segment", x = 60, xend = 60,
         y = 1.20 + 0.028*60, yend = 1.58 + 0.028*60,
         arrow = arrow(length = unit(0.2, "cm"), ends = "both"),
         color = "#888", linewidth = 0.9) +
annotate("text", x = 59, y = (1.20 + 0.028*60 + 1.58 + 0.028*60)/2 - 0.55,
         label = "Intercept\ndifference", color = "#000000", size = 3.2, hjust = .15) +
  scale_y_continuous(limits = c(1.5, 4.0), breaks = seq(1.5,4,0.5)) +
...
The intercept difference has a direct and concrete interpretation. The two regression equations share nearly identical slopes, meaning the test is equally predictive for both groups in terms of how GPA changes per unit of score. What differs is the baseline prediction. At any given test score, a Group B applicant actually tends to earn 0.38 GPA points more than a Group A applicant with the identical score but the combined model does not account for this, predicting them as if they followed Group A's lower regression line. See @tbl-pred-gpa to see how this would pan out in this simulated data.
...
pred_df <- data.frame(
  ...
  Group_B    = c(2.98, 3.12, 3.20, 3.26, 3.40),
  Difference = c(0.38, 0.38, 0.38, 0.38, 0.38)
)

kable(pred_df,
      col.names = c("Test Score", "Group A actual/predicted GPA",
                    "Group B actual GPA", "Gap (B higher by)"),
      align = "cccc",
...
No single analysis constitutes a fairness conclusion. A well-supported fairness argument draws on multiple sources of evidence, each addressing a different aspect of the question. See @tbl-fairness-evidence
